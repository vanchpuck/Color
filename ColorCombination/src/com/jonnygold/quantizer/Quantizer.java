...
public class Quantizer {
	...
	public IsHistogram quantize(IsHistogram histogram, int level){
		...
		histogramData = getHistogramData(histogram);
	...
	private Map<RGBColor, Integer> getHistogramData(IsHistogram histogram) {
		Map<RGBColor, Integer> result = new HashMap<RGBColor, Integer>(histogram.size());
		for(RGBColor color : histogram.getColors()){
			result.put(color, histogram.getCount(color));
		}
		return result;
	}
